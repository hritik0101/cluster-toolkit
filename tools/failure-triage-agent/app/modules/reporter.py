...
- PINNING VS UPGRADING RULE: When resolving version mismatches, prioritize pinning to a known-stable older version rather than blindly upgrading all components to the newest version, unless you can definitively prove the new version satisfies all downstream strict compatibility matrices (e.g., DCGM, CUDA, NCCL).
